...
function parseImages(s: string): string[] {
  const t = s.trim()
  if (!t) return []
  // soporta JSON ["url1","url2"] o CSV "url1, url2"
  try {
...
export default function PropertyCard({ item }: { item: Raw }) {
  ...
  const cover = p.images[0] || '/img/placeholder-property.jpg'

  const priceBadge =
    p.operation?.toLowerCase() === 'alquiler' && p.price_ars
      ? `ARS/mes ${money(p.price_ars, 'ARS')}`
      : p.price_usd ? `USD ${money(p.price_usd, 'USD')}` : 'Precio a consultar'

  return (
    <div className="card overflow-hidden p-0">
      {/* Imagen + badges */}
      <div className="relative">
        <img src={cover} alt={p.title} className="w-full h-48 object-cover" loading="lazy" />
        ...
        <button className="absolute top-2 right-2 bg-white/90 rounded-full p-1.5 shadow-sm" aria-label="favorito">
      ...
      <div className="p-3">
        ...
        <div className="mt-3 flex items-center justify-between">
          ...
          <a className="btn btn-primary" href={`https://wa.me/543834567890?text=Consulta%20por:%20${encodeURIComponent(p.title)}`} target="_blank" rel="noreferrer">
            Consultar
          </a>
        </div>
      </div>
